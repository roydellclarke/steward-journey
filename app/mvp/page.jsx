...
export default function StewardPathMvp() {
  ...
  async function loadProjects(selectProjectId = "") {
    const response = await fetch("/api/stewardpath/projects");
  ...
  async function saveProjectDraft() {
    ...
    try {
      const response = await fetch(`/api/stewardpath/projects/${project.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  async function loadProject(projectId, projectList = projects) {
    ...
    try {
      const response = await fetch(`/api/stewardpath/projects/${projectId}`);
      const payload = await response.json();
      ...
      const latestResponse = await fetch(`/api/stewardpath/projects/${projectId}/analyses/latest`);
  ...
  async function loadAnalysisHistory(projectId) {
    const response = await fetch(`/api/stewardpath/projects/${projectId}/analyses`);
    const payload = await response.json();
    if (response.ok) {
  ...
  async function runAnalysis() {
    ...
    try {
      ...
      const response = await fetch("/api/stewardpath/analyze", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ profile, projectId, intakeState })
      });
      const payload = await response.json();
      if (!response.ok) {
